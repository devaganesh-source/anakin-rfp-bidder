...
def document(title: str, content: str, bid: Bid | None = None) -> HTMLResponse:
    ...
    if bid:
        for slug, label in [(s, data[0]) for s, data in SECTIONS.items()] + [
            ...
            nav += f' <a href="/bids/{bid.id}/{slug}">{label}</a>'
        nav += f'<p>Bid ID: <code id="bid_id">{bid.id}</code></p>'
    return HTMLResponse(
        f"""<!doctype html>
...
<style>
body {{ font: 16px/1.5 system-ui, sans-serif; max-width: 850px;
        margin: 2rem auto; padding: 0 1rem; background: #f5f7fb; color: #17243b; }}
main {{ background: white; padding: 1.5rem; border: 1px solid #d7dfeb; border-radius: 8px; }}
nav a {{ display: inline-block; margin-right: 1rem; color: #174ea6; }}
textarea {{ display: block; width: 100%; box-sizing: border-box; margin: .5rem 0 1rem;
            padding: .75rem; font: inherit; }}
button {{ padding: .65rem 1rem; cursor: pointer; font: inherit; }}
button:disabled {{ cursor: not-allowed; }}
form {{ margin: 1rem 0; }}
pre {{ white-space: pre-wrap; overflow-wrap: anywhere; font: inherit; }}
code {{ overflow-wrap: anywhere; }}
</style></head><body><header><p>Procurement Mock Portal · Local testing only</p>
<nav aria-label="RFP sections">{nav}</nav></header>
...
async def home() -> HTMLResponse:
    return document(
        "RFP submission",
        '<p>Complete Security, Tech Specs, and Pricing, then review your answers.</p>'
        '<form method="post" action="/bids">'
        '<button id="start_bid" type="submit">Start new bid</button></form>',
    )
...
async def review(bid_id: str) -> HTMLResponse:
    bid = get_bid(bid_id)
    content = "".join(
        f'<section><h2>{title}</h2><pre id="review_{input_id}">'
        f'{escape(bid.answers.get(title, "Not provided"))}</pre></section>'
        for title, input_id, _ in SECTIONS.values()
    )
    if bid.submitted:
        content += '<p id="submission_status" role="status">Mock bid submitted.</p>'
    else:
        ...
        content += f'<p id="submission_status" role="status">{status}</p>'
        if ready and not bid.approval.is_set():
            content += (
                '<h2>Human approval</h2><p>Review the answers above before approving. '
                ...
                '<label><input id="confirm_approval" type="checkbox" name="confirm" '
                'value="yes" required> I have reviewed and approve these answers.</label><br>'
                '<button id="approve_bid" type="submit">Approve reviewed answers</button></form>'
            )
        elif not ready:
            content += '<p>Complete every section before requesting approval.</p>'
        content += (
            f'<form method="post" action="/bids/{bid.id}/submit">{hidden_fields(bid)}'
            f'<button id="submit_bid" type="submit"{locked}>Submit mock bid</button></form>'
        )
...
async def section_page(bid_id: str, page: str) -> HTMLResponse:
    ...
    content = (
        ...
        f'<label for="{input_id}">{title}</label><p id="section_hint">{prompt}</p>'
        f'<textarea id="{input_id}" name="{title}" rows="10" '
        f'aria-describedby="section_hint" maxlength="{MAX_ANSWER_LENGTH}" required{readonly}>'
        f'{escape(bid.answers.get(title, ""))}</textarea>'
        f'<button id="save_continue" type="submit"{disabled}>Save and continue</button></form>'
    )
